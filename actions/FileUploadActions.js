...
export const uploadProfilePicture = (dispatch, uri) => {
    ...
    const uploadLocationRef = firebase.storage().ref().child(uploadPath);
...
export const uploadTrackSnapshot = (dispatch, uri, trackKey) => {
    const userId = firebase.auth().currentUser.uid;
    const uploadPath = 'users/' + userId + '/images/tracks/' + trackKey + '/snapshot.jpg';
    const uploadLocationRef = firebase.storage().ref().child(uploadPath);

    const trackRef = firebase.database().ref('users/' + userId + '/tracks/' + trackKey);
